...
const StartPage = () => {
  return (
    <div className={classes.page}>
      ...
      <StartPageItem title={"Что?"} svg={<svg xmlns="http://www.w3.org/2000/svg" width="8" height="13" fill="none">
        <path fill="#000"
      ...
      </svg>} text={"Хз, что тут объяснять. Твердо и четко. Новый сток."} block={false}/>
      <StartPageItem title={"Зачем?"} svg={<svg xmlns="http://www.w3.org/2000/svg" width="8" height="13" fill="none">
        <path fill="#000"
              d="M3.624 8.984c.64 0 1.016-.378 1.016-.838v-.142c0-.66.413-1.085 1.279-1.628C7.135 5.621 8 4.936 8 3.521 8 1.562 6.144.5 4.013.5 1.868.5.45 1.468.1 2.553c-.062.189-.1.378-.1.578 0 .531.439.814.853.814.439 0 .702-.188.928-.472l.225-.283c.44-.684 1.104-1.085 1.931-1.085 1.141 0 1.881.613 1.881 1.51 0 .802-.526 1.192-1.617 1.912-.903.59-1.58 1.215-1.58 2.371v.154c0 .613.363.932 1.003.932ZM3.599 12.5c.727 0 1.354-.543 1.354-1.227s-.614-1.227-1.354-1.227c-.74 0-1.354.554-1.354 1.227 0 .672.626 1.227 1.354 1.227Z"/>
      </svg>} text={"Потому что тупо могу и мне похуй в принципе."} block={false}/>
      <StartPageItem title={"Github"} svg={<svg xmlns="http://www.w3.org/2000/svg" width="12" height="13" fill="none">
        <g clipPath="url(#a)">
      ...
      </svg>} text={"Почему в институте, где тупо все программисты, нет ни одного open-source проекта?"} block={true}/>
    </div>
  );
};
